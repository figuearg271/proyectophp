...
class Contadores extends Controller
{
    ...
    public function index()
    {
        if (isset($_SESSION["rol"])) {
            ...
            $this->getView()->clientes = $this->getModel()->listadoClientes();
            $pagina = 'Contadores/index';
    ...
    {
        if (!empty($_POST)) {
            $this->getModel()->setPServicio($_POST["txtServicio"]);
            $this->getModel()->setContribuyente($_POST["sCliente"]);
            $this->getModel()->setEstado($_POST["estado"]);

            $respuesta = $this->getModel()->save();
            echo $respuesta;
        }
    }
